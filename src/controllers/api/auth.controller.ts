...
export class AuthController {
    ...
    async librarianTokenRefresh(@Req() req: Request, @Body() data: LibrarianRefreshTokenDto): Promise<LoginInfoDto | ApiResponse> {
        ...
        try {
            ...
            throw new HttpException('Bad token found prvo', HttpStatus.UNAUTHORIZED);
        }

        if (!jwtRefreshData) {
            throw new HttpException('Bad token found drugo ', HttpStatus.UNAUTHORIZED);
        }

        if (jwtRefreshData.ip !== req.ip.toString()) {
            throw new HttpException('Bad token found trece', HttpStatus.UNAUTHORIZED);
        }

        if (jwtRefreshData.ua !== req.headers["user-agent"]) {
            throw new HttpException('Bad token found cetvrto', HttpStatus.UNAUTHORIZED);
        }
